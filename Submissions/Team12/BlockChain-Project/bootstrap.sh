...
function removeUnwantedImages() {
  infoln "Skipping removal of chaincode Docker images to preserve chaincode"
  # Commenting out the removal of chaincode images to preserve them
...
function createOrgs() {
  # Always remove the organizations directory to regenerate CA and user data
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert admin Admin@org1.example.com org2
...
function networkUp() {
  ...
  # Always regenerate the organizations directory to ensure fresh CA and user data
  createOrgs

  # Start CA containers if using Certificate Authorities
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
...
function networkDown() {
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG3_FILES} down --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG3_FILES} down
  else
  ...
  # Remove the organizations directory to regenerate CA and user data on next networkUp
  if [ -d "organizations" ]; then
    rm -Rf organizations
    infoln "Removed organizations directory to regenerate CA and user data on next networkUp"
  fi

  # Remove channel artifacts and system genesis block, but preserve ledger and chaincode
  if [ -d "channel-artifacts" ]; then
    rm -Rf channel-artifacts
    infoln "Removed channel-artifacts directory"
  fi

  if [ -d "system-genesis-block" ]; then
    rm -Rf system-genesis-block
    infoln "Removed system-genesis-block directory"
  fi

  # Clear containers but preserve volumes (ledger data)
  clearContainers

  # Preserve chaincode images
  removeUnwantedImages

  infoln "Network stopped. Ledger data (Docker volumes) and chaincode images are preserved."
}
